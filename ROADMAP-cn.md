...
> **当前版本**：v1.7.9-stable（2026-06-25）  
> **下一里程碑**：v1.8.0  
> **愿景**：轻量级 Web 边界安全 — 被动检测 + 半主动响应

---

## v1.7.9-stable — 隔离管道 + 安全加固 ✅ (已完成)

详见 [CHANGELOG-cn.md](./CHANGELOG-cn.md) 和 [RELEASE_NOTES_v1.7.9-cn.md](./RELEASE_NOTES_v1.7.9-cn.md)

---

## v1.8.0 — Web 配置面板 + 威胁检测扩展（2026-Q3）
...
| # | 功能 | 模块 | 优先级 |
|---|------|------|--------|
| — | **Web 配置面板** | Settings 页面 | **P0** |
|   | 通知开关（邮件/微信/Webhook 可视化配置） | 取代手动编辑 .env | P0 |
|   | 监控参数调节（扫描扩展名、排除目录、阈值） | Settings 页面 | P0 |
|   | 系统设置（密码修改、IP 白名单、端口号） | 已有 → 整合到 Settings | P0 |
